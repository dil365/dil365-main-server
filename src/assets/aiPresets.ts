...
export const introducing = (fullname: string, preferred_lang: string | null | undefined) => {
    ...
    str.push(
        `Please speak with me in ${preferred_lang ? preferred_lang : DefaultLang} language, until i say change language.`,
    )
    return str;
}
...
export function resolver(rawValue: { role: 'assistant' | 'user', content: string }) {
...
export function dealing(fullname: string, preferred_lang: string | null | undefined) {
    const topic = [
        ...
        `If user try to ask "who you are?" or ask your version, your model, your ai model" never say who you are and return {success: false}`,
    ...
    str.push(setResponseFormat('JSON').join('\n'));
